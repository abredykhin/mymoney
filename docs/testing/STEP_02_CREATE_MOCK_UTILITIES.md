...
This is the most critical step in the testing infrastructure. It creates working mock utilities that accurately simulate:
- Supabase client with proper query chaining
- Plaid client with correct method signatures
- JWT token generation for authentication testing
- Webhook signature generation
- Test fixtures and helpers

**Why This Step Is Critical:**
The review identified that simplistic mocks will cause tests to fail or produce false positives. This step implements production-ready mocks that work with real code patterns.

---

## Tasks

### Task 2.1: Create Mock Supabase Query Builder

Replace the content of `supabase/functions/_shared/test-utils.ts` with the following:
...
Create a temporary test file `supabase/functions/_shared/test-utils.test.ts`:
...
Deno.test("MockQueryBuilder: should support eq filtering", async () => {
  const mockClient = createMockSupabaseClient({
    mockData: {
      accounts: [
        { id: 1, user_id: 'user-1', name: 'Account 1' },
        { id: 2, user_id: 'user-2', name: 'Account 2' },
      ],
    },
  });
  ...
  assertEquals(data[0].name, 'Account 1');
});

Deno.test("MockQueryBuilder: should support chained filters", async () => {
  const mockClient = createMockSupabaseClient({
    mockData: {
      transactions: [
        { id: 1, amount: 100, user_id: 'user-1' },
        { id: 2, amount: 200, user_id: 'user-1' },
        { id: 3, amount: 50, user_id: 'user-2' },
      ],
    },
  });
  ...
  assertEquals(data.length, 1);
  assertEquals(data[0].id, 2);
});

Deno.test("MockQueryBuilder: should support single()", async () => {
  const mockClient = createMockSupabaseClient({
    mockData: {
      profiles: [{ id: 'user-1', email: 'test@test.com' }],
    },
  });
...
deno test _shared/test-utils.test.ts
```

All tests should pass. Delete the test file after validation:

```bash
rm _shared/test-utils.test.ts
```
...
git commit -m "Create production-ready mock utilities

- Implement MockQueryBuilder with full query chaining support
- Add mock Supabase client with proper method signatures
- Add mock Plaid client matching real SDK
- Add JWT generation for auth testing
- Add webhook signature generation
- Add fixture loader and mock data helpers
...
Proceed to [Step 3: Environment Setup](./STEP_03_ENVIRONMENT_SETUP.md)

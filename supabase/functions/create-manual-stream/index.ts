...
async function createManualStream(
  ...
  const { data: transaction, error: txError } = await supabase
    ...
    .select('name, merchant_name, amount, personal_finance_category, personal_finance_subcategory, account_id')
    .eq('id', transactionId)
    .eq('user_id', userId)
    .single();
  ...
  const streamData = {
    ...
    personal_finance_subcategory: transaction.personal_finance_subcategory || null,

-- REGRESSION FIX: the Sunday-week migration (20260607180000) recreated get_budget_state from the
-- 20260603080001 (pre-paycheck) version, which split income on is_recurring ALONE. That silently
-- reverted 20260605120000, whose whole point was to count a received paycheck as KNOWN income via
-- is_paycheck (originator-id / amount match) even while pending/unlinked. Consequence after the bad
-- deploy: a landed paycheck counted as "extra" again, inflating the pool AND making "Expected
-- paycheck" project a full month of salary on top of the received check (double-counting ~one
-- paycheck).
--
-- This migration restores the correct income split (known = is_recurring OR is_paycheck) AND keeps
-- the Sunday week boundary. It is the 20260605120000 get_budget_state body with only the week
-- derivation changed to Sunday (EXTRACT(dow)). The spendable_income_transactions view, extract_ach_id
-- helper, and get_monthly_income_summary from 20260605120000 are untouched and remain in place.

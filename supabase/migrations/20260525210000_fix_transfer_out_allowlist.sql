-- Fix: TRANSFER_OUT allowlist instead of blanket include.
--
-- The previous migration excluded only TRANSFER_IN, which let through
-- $70k brokerage, $14k Robinhood, and $47k×2 wire transfers — making
-- the monthly spending stat -$178k.
--
-- New rule for TRANSFER_OUT: only include subcategories that represent
-- actual person-level spending:
--   TRANSFER_OUT_ACCOUNT_TRANSFER  → therapy, Venmo, person-to-person
--   TRANSFER_OUT_WITHDRAWAL        → ATM cash
--
-- Everything else in TRANSFER_OUT (investment/retirement funds, wires,
-- inter-account moves) stays excluded.
-- LOAN_PAYMENTS_CAR_PAYMENT (auto lease) and all other non-CC loan
-- payments continue to pass through.
  ...
  WHERE
    ...
    AND (
      (
        ...
        -- For TRANSFER_OUT, only allow account transfers and cash withdrawals
        AND NOT (
...
-- variable_transactions view (drives liquid hero budget widget)
...
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM public.items_table
...
RETURNS TABLE (
    weekday TEXT, date_label DATE, total_spent DOUBLE PRECISION,
...
BEGIN
    ...
    WHERE t.user_id = auth.uid()
      ...
      AND NOT (
        t.personal_finance_category = 'TRANSFER_OUT'
        AND COALESCE(t.personal_finance_subcategory, '') NOT IN (
          'TRANSFER_OUT_ACCOUNT_TRANSFER', 'TRANSFER_OUT_WITHDRAWAL'
        )
      )
    ...
    SELECT
        TO_CHAR(d.date_series, 'Dy'), d.date_series::date,
        COALESCE(dt.t_sum, 0.0)::double precision,
        (d.date_series::date = peak_date),
        COALESCE(pt.merchant, 'No Spend'), pt.category,
        COALESCE(pt.amount, 0.0)::double precision
...
CREATE OR REPLACE FUNCTION public.get_pulse_top_merchants(
    start_date DATE, end_date DATE, lim INTEGER DEFAULT 5
)
RETURNS TABLE (
    merchant_name TEXT, total_spent DOUBLE PRECISION,
    transaction_count BIGINT, personal_finance_category TEXT
)
LANGUAGE plpgsql SECURITY INVOKER SET search_path = public
AS $$
BEGIN
    ...
    SELECT
        ...
        SUM(t.amount)::double precision,
        COUNT(*)::bigint,
        MIN(t.personal_finance_category)
    FROM public.transactions_table t
    WHERE t.user_id = auth.uid()
      AND COALESCE(t.authorized_date, t.date) BETWEEN start_date AND end_date
      AND t.amount > 0

-- Fix: the new `income_basis` OUT column (added in 20260603080000) is ambiguous with
-- profiles_table.income_basis inside the profile SELECT, so every call errored with
-- 42702 "column reference income_basis is ambiguous". Qualify the column reference.
-- Recreates the function (DROP + CREATE) with the qualification.

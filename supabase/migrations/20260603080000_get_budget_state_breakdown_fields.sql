-- T3 follow-up: expose income_basis and upcoming_bills from get_budget_state.
--
-- The "How we got this" breakdown was branching on the legacy spending_plan_mode
-- column while the pool itself is derived from income_basis. For the default user
-- (spending_plan_mode = 'safe_to_spend', income_basis = 'projected') that made the
-- breakdown narrate a cash-on-hand story under an income-discretionary headline, so
-- the steps never reconciled with the total. The client now branches the breakdown
-- on the same basis the RPC used, which means the RPC must return it.
--
-- upcoming_bills is the value the cash_only pool already subtracts internally; the
-- client needs it so the cash-mode breakdown ("Net cash − Upcoming bills − Goals")
-- reconciles exactly with pool_remaining instead of recomputing it client-side.
--
-- Return-type change requires DROP + CREATE (CREATE OR REPLACE cannot alter the
-- output column set).
...
BEGIN
  ...
  SELECT COALESCE(monthly_income, 0), COALESCE(monthly_mandatory_expenses, 0),
         COALESCE(income_basis, 'projected')

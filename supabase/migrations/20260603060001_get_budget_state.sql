...
--
-- Returns the complete budget state in a single round trip, replacing the
-- collection of separate Swift fetches (fetchBudgetSummary, fetchAllPeriodSpend,
-- fetchActualIncome, fetchTotalBalance, upcomingUnpaidBills).
--
-- Validated against prod (user aac9c731…, 2026-06-02):
--   projected pool_remaining = 1,078.54
--   daily_pace = 37.19
--   weekly_pace = 260.34
--
-- Modes:
--   'projected'  → pool_total = max(0, effectiveIncome − mandatory − goals)
--                  pool_remaining = pool_total − spent_mtd
--   'cash_only'  → pool_total = max(0, net_cash − upcoming_bills − goals)
--                  pool_remaining = pool_total   (cash already net of spend)
--
-- prev_*_spent use MTD-aligned windows (same day-of-month as current start),
-- matching the existing get_period_spend_comparison contract.

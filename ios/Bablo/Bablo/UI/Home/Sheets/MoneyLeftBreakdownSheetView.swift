...
struct MoneyLeftBreakdownView: View {
    ...
    private func loadDetails() async {
        ...
        async let spend = homeBreakdownService.fetchHeroSpendBreakdownRows(for: period, trackedCategories: trackedCategories)
        async let income = period == .month ? homeBreakdownService.fetchHeroIncomeRowsForCurrentMonth() : []
        async let excluded = homeBreakdownService.fetchHeroExcludedTransactionRows(for: period)
        async let paid = period == .month ? subService.fetchRecentExpenseMerchantDates() : [:]

...
"Spend" is not one number. There are distinct layers with different filtering, and bugs happen when a widget mixes them. The hard rule (2026-06-03): **every surface that shows a "spent" figure uses layer 2 (`variable_transactions`)** — hero, Money-Left, Cushion, and the whole Pulse Swing. Layer 1 (total `is_spend`) is a base only the "biggest merchants" card still reads. Always know which layer your widget needs.
...
1.  **`is_spend = true` — "money that actually left the account."** Defined on the `transactions` view (see `20260531053522_always_exclude_credit_card_payments.sql`).
    ...
    -   This is the **total real-spend** layer. As of 2026-06-03 no "spent this month" widget reads it directly — only `get_pulse_top_merchants` (the "biggest merchants" card) still uses total spend. Everything that shows a "spent" number uses layer 2.
...
2.  **`variable_transactions` view — the DISCRETIONARY subset of `is_spend`.** (current def: `20260603013224_variable_transactions_mandatory_match_and_count_wires.sql`.)
    ...
    -   This is the **safe-to-spend / cushion** layer and the **single canonical "spent this month"** number. Used by the Liquid Hero, `HeroBudgetCalculator`, `get_period_spend_comparison` (MTD-aligned: previous-period window runs to the *same day of month*, e.g. June 1 vs May 1), the Cushion sheet, **and the entire Pulse "Swing"** — damage-report headline (`get_daily_transaction_stats`), category breakdown (`fetchCategoryBreakdown`), and day-by-day energy (`get_pulse_weekly_energy`), all rebased here on 2026-06-03 (`20260603100000_pulse_spend_uses_discretionary.sql`).

3.  **(retired)** `get_daily_transaction_stats` / `get_pulse_weekly_energy` used to be a third filter set sourcing total spend; as of 2026-06-03 they sum `variable_transactions` (`total_out`) so Pulse matches the hero/Money-Left. `total_in` on the damage report still uses the `is_income` layer.

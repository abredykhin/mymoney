...
"Spend" is not one number. There are distinct layers with different filtering, and bugs happen when a widget mixes them. The hard rule, updated **2026-06-06**:

-   **Discretionary / safe-to-spend surfaces use layer 2 (`variable_transactions`)** — Liquid Hero, Money-Left, Cushion. These subtract bills as obligations *before* showing a "spent" number, so they must never show bills.
-   **Pulse "Swing" surfaces use layer 1 (total `is_spend`)** — the Damage Report headline, "Where it went", and day-by-day Energy. Pulse is a retrospective "where did ALL my money go" view, so it shows total spend; bills are not hidden but broken out into their own **"Bills" bucket** (see `is_mandatory` below) so the categories still sum to the Damage headline. (This **reverses** the 2026-06-03 decision that had pointed Pulse at `variable_transactions`; see `20260606081820_pulse_total_spend_with_bills_bucket.sql`.) The Lineup (biggest merchants) also uses total spend.

Always know which family your widget belongs to. Never mix layers *within one screen*.
...
1.  **`is_spend = true` — "money that actually left the account."** Defined on the `transactions` view (see `20260531053522_always_exclude_credit_card_payments.sql`).
    ...
    -   This is the **total real-spend** layer and what the **Pulse Swing** shows (Damage Report headline `get_daily_transaction_stats.total_out`, "Where it went" `fetchCategoryBreakdown`, day-by-day `get_pulse_weekly_energy`, and `get_pulse_top_merchants`). The `transactions` view also exposes a computed **`is_mandatory`** flag = the exact complement of the `variable_transactions` exclusion (`is_recurring = true` OR merchant matches an `active_mandatory_expense_streams`). `CategoryBreakdownBuilder` routes `is_mandatory` rows into the **`.bills`** `SpendingBucket`, keeping bills visible-but-separate while the total still reconciles. `is_mandatory` is additive/passthrough — discretionary consumers never select it, so layer 2 is unaffected.
...
2.  **`variable_transactions` view — the DISCRETIONARY subset of `is_spend`.** (current def: `20260603013224_variable_transactions_mandatory_match_and_count_wires.sql`.)
    ...
    -   This is the **safe-to-spend / cushion** layer. Used by the Liquid Hero, `HeroBudgetCalculator`, `get_period_spend_comparison` (MTD-aligned: previous-period window runs to the *same day of month*, e.g. June 1 vs May 1), and the Cushion sheet. The Cushion shares `CategoryBreakdownBuilder` with "Where it went" but is sourced from `variable_transactions`, whose rows are by definition never `is_mandatory` — so the Cushion never grows a `.bills` bucket. (The 2026-06-03 migration that had also pointed the Pulse Swing here was reverted 2026-06-06; the Swing is now layer 1 — see above.)

3.  **`.bills` bucket (the Bills category in "Where it went").** Not a DB layer — a display bucket. Any total-spend breakdown row with `is_mandatory = true` is grouped here instead of into a category/Rest, so rent + matched mandatory streams show up as one "Bills" slice that the user can drill into. Bills are *display-only*: they're already subtracted as `monthly_mandatory_expenses` in the pool, so the `.bills` amount must never feed any spend/pool total — it only reconciles the Pulse transaction list. The drill-down (`AllTransactionsView`) honors this via a Pulse-only `showBillsBucket` flag (default off → Home activity unchanged) that adds a Bills chip and excludes `is_mandatory` from the Out/Other/category filters.
...
-   A **Pulse Swing** widget (Damage Report / Where it went / Energy / Lineup) uses total `is_spend`, with `is_mandatory` rows surfaced as the `.bills` bucket — never silently merged into a category. Keep every Swing card on total spend so they reconcile to the Damage headline.
...
-   **External inflows count as income, symmetric with external outflows counting as spend** (set 2026-06-03, `20260603090000_count_external_inflows_as_income.sql`). `is_income` and `spendable_income_transactions` no longer strip brokerage/wire/transfer-named `INCOME` inflows — *any* `INCOME`-category inflow into a non-credit/non-loan account counts. Rationale: brokerage credits (e.g. "Manual CR-Bkrg") often literally fund the spousal wire; counting the wire as spend but not its funding inflow left the inflow counted nowhere and made the pool understate. These land as non-recurring **"extra" income on top of projected salary** (the pool grows by the full inflow) — intended, even though it isn't "earned" income. Consequence: such credits now surface in the Recent feed as **income (+)**, not as a hidden "Transfer".
    -   **DANGER — the `is_income` definition has regressed twice by being recreated from a stale copy.** The `transactions` view's `is_income` must **NOT** carry the `%transfer%/%wire%/%reversal%/%brokerage%/%bkrg%/%schwab%/%moneylink%/%invest%` name exclusion. The current correct body is: `amount < 0 AND account type not credit/loan AND (subcategory = 'TRANSFER_IN_ACCOUNT_TRANSFER' OR category = 'INCOME')` — no name filter. The Pulse migration `20260606081820` re-added the exclusion (copied a pre-2026-06-03 `is_income` while adding the additive `is_mandatory` column); fixed again in `20260608120000_restore_external_inflows_as_income.sql`. Symptom of the regression: the **Damage Report IN** (`get_daily_transaction_stats`, sums `is_income`) drops brokerage/wire credits and disagrees with the home tab "How we got this" extra income. **Any future migration that recreates the `transactions` view MUST base it on the latest definition (currently `20260608120000`), keep the column set/order identical, and keep `is_mandatory` last.**
    -   The two income surfaces use **different sources** and must stay reconciled: the home tab "How we got this" (known/extra split) reads `spendable_income_transactions` via `get_budget_state`, keyed on `category = INCOME` + `is_recurring`/`is_paycheck` — it does **not** read the `is_income` flag. The Damage Report IN reads the `is_income` flag. So an `is_income` regression breaks Damage IN while the home tab stays correct — that divergence is the tell.

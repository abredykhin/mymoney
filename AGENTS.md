...
### Spend Classification Layers (critical — read before any spend/budget widget)

"Spend" is **three different numbers** depending on the layer. Mixing layers inside one widget is the root cause of several budget bugs. Know which layer you need.

1.  **`is_spend = true` = "money that actually left the account."** (`20260531053522_always_exclude_credit_card_payments.sql`.) Includes real outflows Plaid tags as transfers — spousal-support wires (`TRANSFER_OUT_OTHER_TRANSFER_OUT`), ATM, Venmo, therapy. Excludes credit-card payments (`LOAN_PAYMENTS_CREDIT_CARD_PAYMENT`, already counted on the card) and income. This is the **total real-spend** layer; `PulseService.categoryBreakdown` uses it.
2.  **`variable_transactions` = the DISCRETIONARY subset of `is_spend`.** (current def: `20260603013224_variable_transactions_mandatory_match_and_count_wires.sql`.) Additionally drops `is_recurring = true` **and** any txn whose merchant matches an active mandatory expense stream (`active_mandatory_expense_streams`) — the merchant match is a safety net for *pending* recurring bills Plaid hasn't yet linked to their stream (so `is_recurring` is still false), without which this month's rent leaks in and double-counts against `monthly_mandatory_expenses`. **External wires (`TRANSFER_OUT_OTHER_TRANSFER_OUT`, e.g. spousal support) ARE counted here** (reversed 2026-06-02; they were in no bucket before). This is the **safe-to-spend / cushion** layer; the Liquid Hero, `HeroBudgetCalculator`, and `get_period_spend_comparison` use it. `get_period_spend_comparison` is **MTD-aligned** — the previous window ends on the same day-of-month (June 1 vs May 1), not full-prior-month.
3.  **`get_daily_transaction_stats` (Pulse "Damage report")** — `total_out` sums total `is_spend`; `total_in` sums the `is_income` flag (see income layer below).

**Income layer (`is_income`).** Defined on the `transactions` view: `amount < 0 AND account type not credit/loan AND (subcategory = 'TRANSFER_IN_ACCOUNT_TRANSFER' OR category = 'INCOME')` — **no name/merchant filter.** External inflows (brokerage/wire/transfer-named `INCOME` credits like "Manual CR-Bkrg") **count as income**, symmetric with external wires counting as spend (set 2026-06-03, `20260603090000`). The Damage Report IN reads this flag.
-   **DANGER — `is_income` has regressed twice by being recreated from a stale copy.** It must **never** carry the `%transfer%/%wire%/%reversal%/%brokerage%/%bkrg%/%schwab%/%moneylink%/%invest%` name exclusion. The Pulse migration `20260606081820` re-added it (copied a pre-2026-06-03 body while adding the additive `is_mandatory` column); fixed again in `20260608120000_restore_external_inflows_as_income.sql`. Symptom: Damage Report IN drops brokerage/wire credits and disagrees with the home tab. **Any migration recreating the `transactions` view MUST start from the latest def (currently `20260608120000`), keep the column set/order identical, and keep `is_mandatory` last.**
-   The two income surfaces use **different sources**: the home tab "How we got this" (known/extra split) reads `spendable_income_transactions` via `get_budget_state`, keyed on `category = INCOME` + `is_recurring`/`is_paycheck` — **not** the `is_income` flag. Damage IN reads `is_income`. So an `is_income` regression breaks Damage IN while the home tab stays correct — that divergence is the tell.

Rules: (a) a discretionary/cushion screen must use `variable_transactions` for **every** card on it — never one card on `variable_transactions` and another on raw `is_spend`. (b) Spousal-support / external wires **count as variable spend** as of 2026-06-02 (the user wants them in spend; excluding them left them counted nowhere). If Plaid/user later marks them recurring, the mandatory-stream merchant match moves them into obligations instead. (c) Always compare MTD-aligned windows. (d) External inflows **count as income** (`is_income`, no name filter); they land as non-recurring "extra" income unless matched as a paycheck (`is_paycheck`).

### Review Findings And Risks

-   **The Cushion sheet (`TheCushionSheetView` in `TheSwingSheetView.swift`) was reworked (Phases 1–4) so every card reads the discretionary layer over MTD-aligned windows.** Data now flows through `PulseService.fetchCushionData` → `variable_transactions` (NOT `categoryBreakdown`, which stays raw `is_spend` for the Pulse "Swing" sheet), producing `cushionBreakdown` + `cushionDailySeries`; windows come from `HomeView.cushionWindows` via `PreviousPeriodDateRange` (June 1 vs May 1, not full May). The drivers now reconcile to the hero's `roomDelta`, the hero bars show the cushion (room) values so the headline delta is the bar difference, the pace chart is built from the discretionary series (no rescale hack), and `paceSummary` uses a per-period horizon. **Still open (Phase 5):** `previousRoom` remains fabricated as `currentRoom − roomDelta` (assumes constant income/fixed), and a day-1 partial month is a thin comparison — the planned fix is a projection-led framing ("on pace for $X by month-end vs $Y last month"). See `docs/cushion-phase5-proposal.md`.

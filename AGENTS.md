...
3.  **`get_daily_transaction_stats` (Pulse "Damage report")** has its own filter set (drops `TRANSFER`/`LOAN_PAYMENTS`/`Payment`-named/brokerage-`INCOME`). Close to but not identical to the others.

Rules: (a) a discretionary/cushion screen must use `variable_transactions` for **every** card on it — never one card on `variable_transactions` and another on raw `is_spend`. (b) Spousal-support / external wires **count as variable spend** as of 2026-06-02 (the user wants them in spend; excluding them left them counted nowhere). If Plaid/user later marks them recurring, the mandatory-stream merchant match moves them into obligations instead. (c) Always compare MTD-aligned windows.
